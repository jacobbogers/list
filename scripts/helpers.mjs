	...
	for (const path of Object.keys(paths)) {
		...
		if (stars?.length > 1) {
			...
			if (paths[path].length !== 1) {
				errors.push(
					`exact path aliases must have only one value: [${path}] has ${paths[path].length} values`,
				);
			}
		}
	}
...
function walkDirPath(subPaths, basePath, possibleExtensions) {
	...
	const possibles = entries.filter((entry) => {
		...
		// last subpath?
		if (subPaths.length === 0) {
			if (entry.isDirectory() && current === entry.name) {
				// possible directory import
				return true;
			}
			if (!entry.isFile()) {
				// no pipes, file sockets, etc
				return false;
			}
	...
	if (subPaths.length > 0) {
		if (possibles.length) {
			return walkDirPath(
				...
				join(basePath, possibles[0].name),
				possibleExtensions,
			);
		}
		return;
	...
	for (const ext of possibleExtensions) {
		const found = fileImports.find(
			(possible) => extname(possible.name) === ext,
		);
		if (found) {
	...
	for (const dirImport of dirImports) {
		const found = walkDirPath(
			["index"],
			join(basePath, dirImport.name),
			possibleExtensions,
		);
		if (found) {
			return found;
...
function resolveAliasPaths(
	...
	const candidates = wildcard
		.map((wc) =>
			path.startsWith(wc.slice(0, -1)) || path.startsWith(wc.slice(0, -2))
				? wc
				: null,
		)
	...
	for (const candidate of candidates) {
		...
		for (const replace of possibleDirs) {
			...
			try {
				...
				console.log(
					`lstat failed for ${replace} aliased by ${candidate}, with error ${String(err)}`,
				);
				continue;
			}
			...
			const actual = walkDirPath(
				structuredClone(subPathElts),
				dir,
				possibleExtensions,
			);
			if (actual) {
...
export function resolveToFullPath(
	module,
	importStatement,
	base,
	pathsDictionary,
	...
	if (isAliased) {
		const resolved = resolveAliasPaths(
			importStatement,
			pathsDictionary,
			base,
			exact,
		...
		if (resolved === importStatement) {
			// could not be resolved (assume it is node modules or dev forgot to alias in tsconfig.json)
			return resolved;
		}
		if (!resolved) {
			// because of error (some path element replacement does not exist or no read access)
			return importStatement;
		}
	...
	const rc =
		(ext === ""
			? finalImportStatement
			: finalImportStatement.slice(0, -ext.length)) + forceExt;
